...
面向科普演示的月球 Web 系统，使用 Vue、TypeScript 和 CesiumJS。当前为 V0.2 参数模型与科普演示版，不是完整合同交付版。

## 已实现

- 月表与内部统一参数化：半径、厚度、颜色、圈层增删、显隐、完整球/半球/四分之一剖切。
- 3 个早期阶段和现今状态；每个年代可保存参数、恢复默认，支持阶段播放。
- NASA LRO 影像、四种 NASA Trek 在线影像/地形专题/地质图、实际 LOLA 0.25° 三维高程。
- 388 个科普点位，三维点击查询、增删改、GeoJSON 导入导出和本地保存。
- 点位文字、图片、可旋转 GLB 模型、文献和相关链接；缺项不填造。
- 第一视角、基地自动环游、第三人称示意人物；WASD 移动、方向键转头、Shift 加速。
- 场景图片导出，保留数据来源或模拟说明。
